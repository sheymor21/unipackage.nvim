...
function M.check()
    ...
    if picker_status.telescope_enabled then
        if picker_status.telescope_available then
            report(true, "Telescope picker: enabled and available")
        else
            warn("Telescope picker: enabled but not available (install nvim-telescope/telescope.nvim)")
        end
    else
        report(true, "Telescope picker: disabled (using vim.ui)")
    end

    if picker_status.dressing_available then
        report(true, "dressing.nvim: available (enhances vim.ui.select/input)")
    else
        info("Install stevearc/dressing.nvim for enhanced UI")
    end
