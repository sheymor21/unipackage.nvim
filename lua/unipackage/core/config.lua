...
local function validate_config(user_config)
    ...
    if user_config.ui then
        if user_config.ui.telescope ~= nil and type(user_config.ui.telescope) ~= "boolean" then
            table.insert(errors, "ui.telescope must be a boolean")
        end
    end
